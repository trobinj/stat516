...
plot(p)
```
Note that `log` is the "natural" logarithm or base-$e$ logarithm sometimes written as $\ln(x)$ or $\log_e(x)$. 

1. Logarithms of different bases are *proportional*. In general 
$$
  \log_b(x) = c\log_a(x),
$$
where $c = 1/\log_a(b)$. So usually when we are using things like `contrast` or the **emmeans** package to facilitate our inferences the base does not matter. You can use `log2` for $\log_2(x)$ and `log10` for $\log_{10}(x)$, and for an arbitrary base $b$ you can use `log(x,b)` for $\log_b(x)$. Here are few things to remember about logarithms when using them for transformations of explanatory variables.
    ```{r}
