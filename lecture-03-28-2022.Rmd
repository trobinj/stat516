...
## Distributions for Overdispersion

One way to model overdispersion is to assume a model of the form
$$
...
p <- p + geom_jitter(height = 0, width = 0.25)
p <- p + labs(y = "Survival Time (Days)") + theme_classic()
plot(p)
```
...
```{r}
d <- subset(barnacle, DW > 0) # remove observations of zero dry weight to avoid errors
m <- glm(DW ~ F + log(RC) + F:log(RC), family = Gamma(link = log), data = d)
...
abline(2,0)
```
Note: Eliminating a couple of observations due to having a zero dry weight is not of much consequence here since there are so many observations. But if there were fewer observations this would not be a good idea. A better approach would be to just specify the same model using `quasi`.
```{r}
