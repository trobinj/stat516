...
<!-- ```{r} -->
<!-- rmarkdown::paged_table(MASS::whiteside) -->
<!-- ``` -->
<!-- </div> -->

```{r, echo = FALSE, fig.height = 4}
p <- ggplot(MASS::whiteside, aes(x = Temp, y = Gas)) + 
  geom_point(aes(fill = Insul), shape = 21, size = 1.25) +
  labs(x = "Temperature (C)", y = "Gas Consumption (1000s of cubic feet)", fill = "Insulation:") +
  scale_fill_manual(values = c("black","white")) + ylim(0,8) +
  theme_minimal() + theme(legend.position = "inside", legend.position.inside = c(0.8, 0.8)) +
  ggtitle("Gas Consumption by Temperature and Insulation")
plot(p)
```
...
p <- p + theme_minimal() + theme(legend.position = "inside", legend.position.inside = c(0.8, 0.8))
