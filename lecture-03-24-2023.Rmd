...
Here increasing the base-2 logarithm of dose by one unit is the same thing as *doubling* dose, and so the effect on the odds ratio of doubling dose will be the same regardless of what you double (e.g., 0.05 to 1 ppm, 0.1 to 0.2 ppm, etc.).[^doubling] How is this odds ratio interpreted?
    ...
    **Solution**: We can estimate the model as follows.
    ...
    cbind(summary(m)$coefficients, confint(m))
    ```
    Here is how to estimate the odds ratio for the effect of doubling the dose.
    ```{r}
    trtools::contrast(m,
      a = list(Dose = 0.2),
...
0. Estimate the odds and probability of tumor development at each value of dose used in the study for any of the three models.
    ...
    **Solution**: For the odds we need to use `contrast` from **trtools**. I am going to use the model from the previous problem.
    ```{r}
    trtools::contrast(m, a = list(Dose = c(0.01,0.025,0.05,0.1,0.25)),
      cnames = c(0.01,0.025,0.05,0.1,0.25), tf = exp)
    ```
    There are several ways to estimate the probabilities.
    ```{r}
    trtools::contrast(m, a = list(Dose = c(0.01,0.025,0.05,0.1,0.25)),
      cnames = c(0.01,0.025,0.05,0.1,0.25), tf = plogis)
    emmeans(m, ~Dose, type = "response")
    ```
    
Here are plots of the three models we considered for the aflatoxicol data.
...
p <- ggplot(d1, aes(x = yhat, y = residual)) + theme_classic() + 
  geom_point(alpha = 0.5) + ylim(-8,6) + 
  geom_hline(yintercept = c(-2,2), linetype = 3) + 
  labs(x = "Predicted Log-Odds", y = "Studentized Residual")
p1 <- p + ggtitle("Dose")

p <- ggplot(d2, aes(x = yhat, y = residual)) + theme_classic() + 
  geom_point(alpha = 0.5) + ylim(-8,6) + 
  geom_hline(yintercept = c(-2,2), linetype = 3) + 
  labs(x = "Predicted Log-Odds", y = NULL)
