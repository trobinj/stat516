...
## Review of Linear Functions of Model Parameters

Consider a regression model with parameters $\beta_0, \beta_1, \dots, \beta_k$. Sometimes our inferences concern a *linear function* of the form
$$
  \ell = a_0\beta_0 + a_1\beta_1 + a_2\beta_2 + \cdots + a_k\beta_k + b,
$$
where $a_0, a_1, \dots, a_k$ and $b$ are user-specified coefficients (frequently $b=0$, and to simplify what follows assume $b=0$). Often (but not always) quantities of interest can be expressed as linear functions.
...
\end{cases}
$$
Several quantities that might be of interest can be written as linear functions. The `lincon` and `contrast` functions facilitate inferences regarding linear function. First we specify the model.
```{r}
m <- lm(Gas ~ Insul + Temp + Insul:Temp, data = MASS::whiteside)
summary(m)$coefficients
```
Now consider the following linear functions.

Rate of change before insulation: $\ell = 0\beta_0 + 0\beta_1 + 1\beta_2 + 0\beta_3 = \beta_2.$
```{r}
library(trtools)
...
Rate of change after insulation: $\ell = 0\beta_0 + 0\beta_1 + 1\beta_2 + 1\beta_3 = \beta_2 + \beta_3.$
```{r}
...
Expected gas consumption before insulation at 5C: $\ell = 1\beta_0 + 0\beta_1 + 5\beta_2 + 0\beta_3 = \beta_0 + 5\beta_2.$
```{r}
lincon(m, a = c(1,0,5,0))
...
```
The `margeff` function does the same thing but computes the derivative above numerically. 
```{r}
...
lincon(m, a = c(0,0.25))
```
It is useful to note that when the function is linear, `dmethod` will produce the same results as `lincon`. But it provides a slightly different interface which can be useful since it does not require us to figure out the coefficients $a_0, a_1, \dots, a_k$. 

**Example**: Consider again the model for the `whiteside` data.
```{r}
m.gas <- lm(Gas ~ Insul + Temp + Insul:Temp, data = MASS::whiteside)
```
The rate of change *after* insulation is $\beta_2 + \beta_3$. 
```{r}
lincon(m.gas, a = c(0,0,1,1))
contrast(m.gas,
  a = list(Insul = "After", Temp = 1),
  b = list(Insul = "After", Temp = 0))
dmethod(m.gas, pfunc = "b2 + b3", pname = c("b0","b1","b2","b3"))
```

**Example**: Returning the the `bliss` data and model, consider the *discrete* marginal effect of increasing dose from 50 to 60 mg/liter. This is 
$$
...
library(GLMsData)
data(paper) 
m <- lm(Strength ~ Hardwood + I(Hardwood^2), data = paper)
summary(m)$coefficients
d <- data.frame(Hardwood = seq(1, 15, length = 100))
d$yhat <- predict(m, newdata = d)
p <- ggplot(paper, aes(x = Hardwood, y = Strength)) + 
...
dmethod(m, pfunc = "(2*b2*b3 - b1*b5)/(b5^2 - 4*b3*b4)",
  pname = c("b0","b1","b2","b3","b4","b5"))
```
There are functions from several other packages that will implement the delta method in various ways. Most are not as flexible as the `dmethod` function from the **trtools** package, but the `hypotheses` function from the **marginaleffects** package is quite flexible. 

## Using `dmethod` for Linear Functions of Model Parameters

We can use the delta method for *linear* functions of model parameters. It essentially provides an alternative interface to specifying the function. 
...
```{r}
lincon(m, a = c(0,0,1,0))
dmethod(m, pfunc = "b2", pname = c("b0","b1","b2","b3"))
```
2.$\!$ Rate of change after insulation: $$\ell = 0\beta_0 + 0\beta_1 + 1\beta_2 + 1\beta_3 = \beta_2 + \beta_3.$$
```{r}
lincon(m, a = c(0,0,1,1))
dmethod(m, pfunc = "b2 + b2", pname = c("b0","b1","b2","b3"))
```
3.$\!$ Expected gas consumption before insulation at 5C: $$\ell = 1\beta_0 + 0\beta_1 + 5\beta_2 + 0\beta_3 = \beta_0 + 5\beta_2.$$
```{r}
lincon(m, a = c(1,0,5,0))
dmethod(m, pfunc = "b0 + 5*b2", pname = c("b0","b1","b2","b3"))
```

Note that `dmethod` does not automatically use finite degrees of freedom (where applicable). The reason is that when we use the delta method, we are relying on an approximation that does not use the $t$-distribution. But to make the method match `lincon`, we can specify it manually.
```{r}
dmethod(m, pfunc = "b0 + 5*b2", pname = c("b0","b1","b2","b3"), df = 52)
```
