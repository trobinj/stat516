...
p <- ggplot(bone, aes(x = age, y = spnbmd)) + 
  ...
  geom_line(aes(color = sex, group = idnum), alpha = 0.25) +
...
p <- ggplot(bone, aes(x = age, y = spnbmd)) + 
  geom_point(aes(color = sex), alpha = 0.25) + theme_classic() + 
  geom_line(aes(color = sex, group = idnum), linewidth = 0.5, alpha = 0.25) +
  labs(x = "Age (years)", color = "Sex",
    y = "Relative Spinal Bone Mineral Density") + 
  geom_line(aes(y = yhat, color = sex), data = d)
plot(p)
...
m <- scam(PA ~ s(elevation, bs = "cv", m = 2), family = binomial, data = anoctua)
...
plot(m, select = 2)
```
**Example**: Let's revisit the crab experiment.
```{r}
library(Stat2Data)
data(CrabShip)

m <- gam(Oxygen ~ Noise + s(Mass, by = Noise), data = CrabShip)
summary(m)

d <- expand.grid(Mass = seq(22, 85, length = 100), Noise = c("ambient","ship"))
d$yhat <- predict(m, newdata = d)
...
library(emmeans)
pairs(emmeans(m, ~Noise|Mass, at = list(Mass = c(40,50,60))),
  reverse = TRUE, infer = TRUE)
```

**Example**: How about the fruit fly data (a "GAM ANCOVA")?
```{r}
library(faraway)
