...
trtools::contrast(m, tf = exp,
  ...
  cnames = paste(c(0.025,0.05,0.1,0.25), "vs 0.01"))
```
But be careful using the log link function with binomial regression, particularly with quantitative explanatory variables. 
```{r, warning = FALSE}
m <- glm(cbind(Tumor, Total - Tumor) ~ Dose, 
  family = binomial(link = log), data = ex2116)
d <- data.frame(Dose = seq(0, 0.30, length = 100))
d$yhat <- predict(m, newdata = d, type = "response")
