...
$$
  ...
  \end{cases}
$$
Suppose we want to make inferences about the difference in the expected weight change under the cognitive-behavioral and control conditions. Thus
\begin{align*}
  E(Y_a) & = \beta_0, \\
  E(Y_b) & = \beta_0 + \beta_1,
\end{align*}
...
```
`contrast` includes an optional `cnames` (contrast names) argument to label the output for clarity.
```{r}
contrast(m, 
