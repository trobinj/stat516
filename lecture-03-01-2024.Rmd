...
$$
if we treat $S_i$ as fixed. So the variance of a rate is *inversely proportional* to $S_i$. 

For example, suppose $E(R_i) = E(R_{i'}) = 0.5$, but $S_i = 2$ and $S_{i'} = 100$ so that $R_i = C_i/2$ and $R_{i'} = C_{i'}/100$. Then 
$$
...
```
When using other tools like `contrast` or functions from the **emmeans** package, be sure to specify the offset (if necessary). Here are the rate ratios for the treatment.
```{r}
trtools::contrast(m,
...
```
When using `predict` we need to be sure to also include the offset amount.
```{r}
