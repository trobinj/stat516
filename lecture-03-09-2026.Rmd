...
plot(p)
```
Estimated probabilities, with confidence intervals, can also be obtained using `contrast` or `glmint`. Note that the function $e^x/(1+e^x)$ is known to R as `plogis`. 
```{r}
...
plot(p)
```
We can also use the **emmeans** package to get estimated probabilities.
```{r}
library(emmeans)
emmeans(m, ~concentration, at = list(concentration = c(50,60,70)), type = "response")
```
...
```
Now using the **emmeans** package.
```{r}
pairs(emmeans(m, ~concentration, at = list(concentration = c(2,1))), 
  type = "response", infer = TRUE)
```
Although `type = "response"` gave probabilities when using `emmeans` along, when using it with `pairs` it is assumed that we want odds ratios, *not* ratios of probabilities (also known as a "risk ratio"). 
...
```
The odds ratios are then simply a ratio of these odds. Here is how we can obtain odds ratios using the **emmeans** package.
```{r}
pairs(emmeans(m, ~trout), type = "response", infer = TRUE)
pairs(emmeans(m, ~trout), type = "response", infer = TRUE, reverse = TRUE)
```
