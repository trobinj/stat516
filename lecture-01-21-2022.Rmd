...
0. The change in expected gas consumption (before and after) for a 10 degree increase in temperature. 
...
3. The change in expected gas consumption (before and after insulation) for a 10 degree increase in temperature are $10\beta_2$ and $10\beta_2 + 10\beta_3$, respectively.
\vspace{2cm}
      ```{r}
      lincon(m, a = c(0,0,10,0), b = 0) # before
      lincon(m, a = c(0,0,10,10), b = 0) # after
      ```
...
5. The change in the expected gas consumption from before to after insulation at 5C is $\beta_1 + \beta_35$. How is this expressed as a linear combination of $\beta_0, \beta_1, \beta_2, \beta_3$?
\vspace{2cm}
      ```{r}
