...
```{r}
cycles$subject <- 1:nrow(cycles) # just for clarity
...
```
Let $P(Y_{it} = 1) = \pi_{it}$ be the probability that the $i$-th mother will become pregnant on the $t$-th cycle *given* that they did not become pregnant on an earlier cycle. We could consider a logistic regression model such that
$$
