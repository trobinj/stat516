...
1. `lm` (**l**inear **m**odel) is a *function* to which we have provided two *arguments*: `formula` and `data`. Other arguments can be found using `args(lm)`, some of which we will use later.
      ```{r}
      args(lm)
      ```
...
```{r, echo = FALSE}
if (exists("anorexia")) rm(anorexia)
```
...
Here's another parameterization. Including the term `0` or `-1` in the model formula argument suppresses the inclusion of the $\beta_0$ parameter (again, it is symbolic, not literal).[^intercept] 
```{r}
