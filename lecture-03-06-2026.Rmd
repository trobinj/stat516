...
$$
if we treat $S_i$ as *fixed* (like we do $x_{i1}, x_{i2}, \dots, x_{ik}$). But rather than using $R_i$ as the response variable we can use $C_i$ as the response variable in a Poisson regression model where
$$
...
```
Also for rate ratios the size of the offset does not matter since it "cancels-out" in the ratio. Also since there is no interaction in this model which means the rate ratio does not depend on location, we can omit it when using `emmeans` (but not `contrast`).
```{r}
pairs(emmeans(m, ~treatment, type = "response"), infer = TRUE)
```
Use `reverse = TRUE` to "flip" the rate ratio. 
```{r}
