...
But we need to be very careful about how we interpret parameters for other linear models.
...
$$
where $\log_2$ is the base-2 logarithm, and assuming that $x > 0$. Here $\beta_1$ is the change in $E(Y)$ if we increase $\log_2(x)$ by one unit, not $x$. If we increase $x$ by one unit we have a change in the expected response of 
$$
  \beta_0 + \beta_1\log_2(x+1) - [\beta_0 + \beta_1\log_2(x)] = \beta_1[\log_2(x+1) - \log_2(x)].
$$
So the change in the expected response if we increase $x$ by one unit *depends on the value of* $x$. The above is also true for any base of logarithm. But for $\log_2$ we have that $\beta_1$ is the change in $E(Y)$ if we *double* $x$. That is,
$$
  E(Y) = \beta_0 + \beta_1\log_2(2x) = \beta_0 + \beta_1\log_2(x) + \beta_1.
$$
We'll discuss log transformations later in the course. 
