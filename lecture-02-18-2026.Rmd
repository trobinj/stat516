...
```{r, fig.height = 6}
library(dplyr)
cars <- read.csv("https://raw.githubusercontent.com/trobinj/stat516/refs/heads/gh-pages/cars.csv") 
cars <- cars |> select(car, mpg, weight, hp, ratio)
m <- lm(mpg ~ weight + hp + ratio, data = cars)
cars$yhat <- predict(m)
...
**Solutions**: Modify the model. Drop offending observation(s) if and only if you can justify restricting the scope of the model.
