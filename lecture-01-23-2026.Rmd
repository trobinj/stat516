...
0. The expected gas consumption before and after insulation at 5C.

0. The difference in expected gas consumption between before and after insulation at 5C.
...
```{r}
m <- lm(Gas ~ 0 + Insul + Insul:Temp, data = whiteside)
summary(m)$coefficients
```
...
$$
where $a_0, a_1, \dots, a_k$ and $b$ are specified coefficients. There are several ways to do this in R. One way is to use the `lincon` function (for **lin**ear **con**trast) from the **trtools** package. 
