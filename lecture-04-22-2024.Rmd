...
In general, the fixed effects approach does not permit proper inferences for explanatory variables that have a fixed effects variable nested within them. We are limited to inferences that are *crossed* with the fixed effect.
...
pairs(emmeans(m, ~ treatment, type = "response"), infer = TRUE, reverse = TRUE)
```
But there is a way to estimate the average odds ratio using the **emmeans** package using `regrid`.
```{r}
