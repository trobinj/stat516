...
<!-- This lecture does not take the entire 50 minutes, even when covering the last part of the last lecture if it was not finished previously. It might be useful to show *mathematically* how the odds ratios are or are not invariant to the other variable when an interaction is present/absent, or when log transformations are used. -->
...
$$
  ...
  \end{cases}    
$$
and where $d_i = x_{i3}$ is the deposit. Note that we could also write the model as
$$
...
$$
  O_i = 
  \begin{cases}
    \exp(\beta_0 + \beta_3d_i),           & \text{if the $i$-th observation of insecticide is g-BHC}, \\
    \exp(\beta_0 + \beta_1 + \beta_3d_i), & \text{if the $i$-th observation of insecticide is both}, \\
    \exp(\beta_0 + \beta_2 + \beta_3d_i), & \text{if the $i$-th observation of insecticide is DDT}. 
  \end{cases}    
$$
We could plot the estimated *odds* of death as a function of deposit and insecticide type. 
```{r}
d <- expand.grid(deposit = seq(2, 8, length = 100),
...
A model for the *odds* of death can be written as
$$
  O_i = e^{\beta_0}e^{\beta_1x_{i1}}e^{\beta_2x_{i2}}e^{\beta_3x_{i3}}e^{\beta_4x_{i4}}e^{\beta_5x_{i5}}
$$
where $x_{i1}$ and $x_{i2}$ are indicator variables for the insecticides `both` and `DDT`, respectively, $x_{i3}$ is deposit, $x_{i4} = x_{i1}x_{i3}$, and $x_{i5} = x_{i2}x_{i3}$. This can be written case-wise as
$$
  O_i = 
  \begin{cases}
    e^{\beta_0}e^{\beta_3d_i},            & \text{if the $i$-th observation of insecticide is g-BHC}, \\
    e^{\beta_0}e^{\beta_1}e^{\beta_3d_i}e^{\beta_4d_i}, & \text{if the $i$-th observation of insecticide is both}, \\
    e^{\beta_0}e^{\beta_2}e^{\beta_3d_i}e^{\beta_5d_i}, & \text{if the $i$-th observation of insecticide is DDT}, 
  \end{cases}    
$$
and where $d_i = x_{i3}$ is the deposit. This implies that an odds ratio for the effect of dose depends on the insecticide, and an odds ratio for the effect of an insecticide depends on the dose.
```{r}
