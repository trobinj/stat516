...
<!-- Introduce using emtrends to estimate rate and odds ratios, along with the pairs function to compare them. Note that you need to have type = "response" and tran = "log" (for odds ratios at least) to do this. -->

## Using the **emmeans** Package for Poisson and Logistic Regression

The **emmeans** package can be used to produce some of the same inferences that are obtained using `contrast` with respect to estimated expected rates/probabilities as well as rate/odds ratios.
...
```
And we can do it using `emmeans` if we specify `type = "response"` and use the `at` argument to specify an quantitative explanatory variables. 
```{r}
library(emmeans)
...
emmeans(m, ~concentration|strain, at = list(concentration = c(2,1)), type = "response")
pairs(emmeans(m, ~concentration|strain, at = list(concentration = c(2,1)), type = "response"))
```
We can also make inferences to *compare* the two rate ratios.
```{r}
trtools::contrast(m, tf = exp,
  a = list(strain = "b", concentration = 2),
  b = list(strain = "b", concentration = 1),
  u = list(strain = "a", concentration = 2),
  v = list(strain = "a", concentration = 1))
pairs(pairs(emmeans(m, ~concentration|strain, 
  at = list(concentration = c(2,1)), type = "response")), by = NULL, reverse = TRUE)
```
This is a ratio of rate ratios. To just get the test statistic you can leave off `type = "response"`. 
```{r}
pairs(pairs(emmeans(m, ~concentration|strain, 
  at = list(concentration = c(2,1)))), by = NULL, reverse = TRUE)
```
...
```{r}
emmeans(m, ~deposit|insecticide, at = list(deposit = c(2,1)), type = "response")
...
```
Here I have left off `type = "response"`. Including it will give ratios of odds ratios, which is a bit confusing, but if all we care about is whether the odds ratios are significantly different this is sufficient. Note that to avoid controlling for familywise Type I error rate include the option `adjust = "none"`. 
...
d <- expand.grid(city = levels(eba1977$city), age = levels(eba1977$age))
cbind(d, trtools::glmint(m.b, newdata = d))
d <- expand.grid(city = levels(eba1977$city), age = levels(eba1977$age), pop = 1)
...
```{r}
d <- expand.grid(age = levels(eba1977$age), city = levels(eba1977$city), pop = 1)
...
p <- ggplot(eba1977, aes(x = city, y = cases/pop)) +
  ...
  labs(x = "City", y = "Cases of Lung Cancer Per Captia") +
  theme_minimal()
plot(p)
p <- ggplot(eba1977, aes(x = age, y = cases/pop, color = city)) +
  ...
  labs(x = "Age Range", y = "Cases of Lung Cancer Per Captia", 
       color = "City") + 
  theme_minimal() + theme(legend.position = c(0.9,0.3))
plot(p)
```
...
plot(p)
```
The problem is that the estimation procedure "wants" the curve to be a step function, but that only occurs as $\beta_1 \rightarrow \infty$, and the value of $x$ where the estimated expected response is 0.5 equals $-\beta_0/\beta_1$, and for the step function that would be 4.5, so the estimation procedure "wants" the estimate of $\beta_0$ to be $-\beta_15.5 = -\infty$. This is called *separation*. It is fairly obvious with a single explanatory variable, but much less so with multiple explanatory variables. The example above shows *complete separation* because we can separate the values of $y$ based on the values of $x$. *Quasi-separation* occurs when this is almost true as in the following example.
```{r}
...
p <- ggplot(mydata, aes(x = x, y = c/m)) + theme_minimal()
p <- p + geom_point() + geom_line(aes(y = yhat), data = d)
plot(p)
```
...
0. For logistic regression (or similar models) a "penalized" or "bias-reduced" estimation method can be used (see the **logistf** and **brglm** packages). 
