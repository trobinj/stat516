...
Odds are sometimes stated in "fractional form" as two numbers separated by a colon or other character (e.g., an odds of 1.5 might be written as "3:2" or "three to two").
```{r, echo = FALSE}
...
```
Let $O_i$ be the odds for the $i$-th observation. Then $O_i = p_i/(1-p_i)$ and $p_i = O_i/(1 + O_i)$. Note that while $0 \le p_i \le 1$ but $0 \le O_i \le \infty$. 
...
p <- p + labs(x = "Concentration of Carbon Disulphide (mg/liter)",
...
p <- p + geom_point() + ylim(0, 0.5) + coord_flip() + theme_minimal()
p <- p + geom_label_repel(aes(label = proportion), box.padding = 0.75)
p <- p + labs(x = "Presence/Absence of Brook Trout",
