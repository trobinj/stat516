...
$$
	E(Y_i) = \beta_0 + \beta_1 x_i + \beta_2 I(x_i < \delta)(x_i - \delta),
$$
where $Y_i$ and $x_i$ are the fire ant count and age of attractant, respectively, and $I$ is an *indicator function* defined as
$$
	I(x_i < \delta) = 
	\begin{cases}
	 	1, & \text{if $x_i < \delta$}, \\
...
**Note**: Don't confuse the *indicator function* $I$ with the "inhibit" function `I` in R. An *indicator function* is a *mathematical function* that returns a 1 or 0 depending on if its argument is true or false, respectively. The inhibit function is a R function that is used to force R to treat something "as is" --- usually in a model formula argument. 
...
```
Note that we can write the indicator function $I(x_i < 40)$ as `(day < 40)` in R.
```{r}
...
$$
	E(Y_i) = \beta_0 + \beta_1 x_i + \beta_2 I(x_i < \delta)(x_i - \delta)g_i,
$$
where 
$$
  g_i = 
...
$$
where $x_{i1} = x_i$ (day) and $x_{i2} = I(x_i < \delta)(x_i - \delta)g_i$, provided we *know* $\delta$. But what if $\delta$ is unknown and is to be estimated? Then we have a *nonlinear* model.
...
cbind(summary(m)$coefficients, confint(m))
```
The `contrast` function does not work with a `nls` object, but we can use `lincon` provided that the quantity of interest is a linear combination of parameters. For example, recall that the model can be written as
$$
  E(Y_i) = 
	\begin{cases}
		\beta_0 - \beta_2 \delta + (\beta_1 + \beta_2)x_i, 
...
$$
	...
	I(\delta - \gamma \le x \le \delta + \gamma) + I(x > \delta + \gamma)(x - \delta).
$$
