...
3. Use a robust estimator of the standard errors (i.e., heteroscedastic consistent standard errors).
...
**Example**: Consider the model for the rotifer data. Here the variance is proportional to $E(Y_i)[1 - E(Y_i)_i]/m_i$ (recall that $m_i$ is the "total possible" for the counts). To estimate this model using iteratively weighted least squares we use weights of
$$
